...
| `CURSOR_PROXY_THINKING_BLOCKS` | _(空=off)_ | `1` 时把 Cursor 上游真实 `thinking_delta` 包装成 proxy-local `thinking` 内容块供 Claude Code 展示。签名以 `proxy-local-thinking-v1.` 开头，仅适用于本代理会话，不是 Anthropic 原生 signed thinking |
